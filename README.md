...
> TTL to 2 minutes (long enough to comfortably click around the page by
> hand without racing the clock) and adds
> `scripts/demo-stale-content.sh`, a scripted walkthrough that
> deliberately reproduces stale content: it warms the cache, writes
> new data, and shows NGINX still serving the old page for a while
...
3. **Wait it out.** On this branch the TTL is 2 minutes — wait that long
   after the write and repeat the GET through NGINX — `X-Cache-Status:
   EXPIRED`, and the new item appears in the HTML. (`main` uses a much
   shorter 15s TTL if you want to see this faster.)
...
- `demo/stale-content` *(this branch)* — same architecture, tuned to a
  2-minute TTL and a scripted walkthrough
  (`scripts/demo-stale-content.sh`) that deliberately reproduces the
  staleness window end to end.
